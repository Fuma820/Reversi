...
//クラスインスタンス生成
var rectDrawer = new RectDrawer(context);
var circleDrawer = new CircleDrawer(context);
var triangleDrawer = new TriangleDrawer(context);
...
function updateField() {
    ...
    var y = 0;
    // 盤面表示
    rectDrawer.draw(BG_COLOR, 0, 0, size * 2, size * Math.sin(Math.PI / 3) * 2);
    nextList.length = 0;
    for (var i = 0; i < row; i++) {
        for (var j = 0; j < column; j++) {
            x = j * tSize / 2;
            ...
            if (checkOnField(j, i)) {
                triangleDrawer.draw(bgColor, x, y, direction);
                if (field[j][i] == 1) {
                    circleDrawer.draw(COLOR_1, x, y, 0.9 * cellSize / 2);
                } else if (field[j][i] == 2) {
                    circleDrawer.draw(COLOR_2, x, y, 0.9 * cellSize / 2);
                } else if (field[j][i] == 3) {
                    circleDrawer.draw(COLOR_3, x, y, 0.9 * cellSize / 2);
                }
            }
        }
    }
